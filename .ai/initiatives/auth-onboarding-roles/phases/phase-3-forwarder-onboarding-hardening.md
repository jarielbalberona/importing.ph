...
Prove or minimally harden forwarder onboarding from signed-in Clerk user to PostgreSQL company/member records and forwarder redirect.

## Scope

- Forwarder branch of `lib/onboarding.ts`.
- `app/onboarding/actions.ts`.
- `app/onboarding/page.tsx` only if validation or form wiring is broken.
- `scripts/prove-onboarding.ts` for forwarder proof/idempotency coverage.
- `lib/routes.ts` for forwarder destination only if current routing is wrong.
...
- Importer-only changes unless shared onboarding code requires it.
- Forwarder service profiles.
- Manual approval or document verification.
- Shipment request browsing.
- Quotes.
- Messaging.
- Tenant abstractions.
...
- Verify forwarder validation requires role, full name, and company name.
- Verify forwarder write creates `user_profiles`, `forwarder_companies`, and `forwarder_members`.
- Verify forwarder write is transactional.
- Verify `forwarder_members.member_role` is `owner`.
- Verify duplicate/retry onboarding cannot create duplicate companies or memberships.
- Verify duplicate/retry onboarding cannot silently switch an existing role.
- Verify forwarder redirect target is `/app/forwarder/requests`.
- Document that forwarder trust status is not currently modeled; do not add it without a product decision.
...
- Forwarder proof passes.
- Forwarder user profile, company, and member IDs are produced by proof output.
...
- Forwarder onboarding idempotency bug.
- Forwarder proof-script coverage gap.
- Forwarder redirect mismatch.
- Minor validation/type/import issue in touched scope.

Hard-stop instead of repairing when:

- Fix requires manual approval, trust status, or verification product decisions.
- Fix requires storing role/profile data in Clerk metadata.
- Fix requires tenant/workspace abstraction.
